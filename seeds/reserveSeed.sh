...
node generators/datesGenerator.js --output=$FILEPATH --lines=$LINES

### Import Our posts.csv file to seed Database ###
psql -U $USER -d $DATABASE -c "COPY dates (listing_id, firstDate, lastDate) FROM '$FILEPATH' CSV HEADER";